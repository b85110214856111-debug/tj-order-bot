    ...
    with zipfile.ZipFile(zip_buffer, "w", zipfile.ZIP_DEFLATED) as z:
        ...
        for p in photos[1:]:
            ...
            if start and end:
                ...
                if photo_date < start or photo_date > end:
                    continue
...
def parse_order_blocks(text):
    ...
    for line in lines:
        ...
        if temp_dates:
            ...
            current_dates = temp_dates


            if index < len(tokens):

                current_customer = tokens[index]
        ...
        if not current_dates or not current_customer:
...
def parse_multi_customer_order(text):
    ...
    for line in lines:
        ...
        # 日期在前
        # =========================
        if tokens and parse_date(tokens[0]):
        ...
        # 客戶在前
        # =========================
        elif len(tokens) > 1 and parse_date(tokens[1]):
        ...
        for u in sorted(
            ...
            if remain_unit_note.startswith(u):
                ...
                unit = u
                remain_unit_note = remain_unit_note[len(u):].strip()
                break
...
def query_order(text):
    ...
    # 日期自動轉完整年月日
    d = parse_date(keyword)

    if d:
        keyword = format_date(d)
    ...
    for r in result[:50]:
        ...
        lines.append(
            f"單號:{r[0]} 日期:{display_date(r[2])} 客戶:{r[3]} 商品:{r[4]} 數量:{qty}{unit} 配送:{delivery} 備註:{note}"
        )
    return "\n".join(lines)
